# Team & Calender Automation

## About
These projects were built to automate internal workflows, reduce manual scheduling overhead, and synchronize project due dates into Google Calendar for centralized tracking using Google Workspace automation tools.

## Tech Stack
- Google Apps Script (JavaScript)
- Google Sheets
- Google Forms
- Google Calendar
- Gmail API (via Apps Script)

## Features Overview
- Automatically creates Google Calendar events from form submissions  
- Supports both all-day and timed absences  
- Color-coded events per team member  
- Input validation for missing or incorrect data  
- Built using Google Apps Script  

---
    
## Tool # 1. Automated Team Logistics Suite  

### Problem
Team members were not consistently logging time off in a shared calendar, leading to scheduling conflicts and frequent manual follow-ups.

### Solution
Built a Google Forms + Apps Script automation system that converts form submissions into calendar events and generates weekly summary emails, keeping the team calendar automatically up to date.

### Impact
- Reduced manual effort required to track team availability  
- System ran reliably for 7+ months  
- Automated weekly scheduling summary emails  

### Workflow

```text
Google Form
↓
Google Sheet
↓
Apps Script
↓
Google Calendar
↓
Gmail  
```
---

## Tool #2. Automated Project Tracker  

### Problem
Project deadlines were being tracked manually in Google Calendar, causing delays, inconsistencies, and misalignment with spreadsheet data.

### Solution
Developed a Google Apps Script system that reads project data from a spreadsheet and automatically generates calendar events while maintaining a visual schedule inside Google Sheets.

### Impact
- Eliminated manual calendar updates  
- Improved synchronization between Sheets and Calendar  
- Provided real-time visibility of upcoming workloads  

### Features
- Automated active project tracking  
- Dynamic calendar generation inside Google Sheets  
- Cross-month workload visualization  
- Google Calendar sync with duplicate prevention  
- Priority-based visual encoding  

### Workflow

```text
Google Sheets
↓
Data Processing
↓
Calendar Visualization
↓
Google Calendar 
```
---
